...
- EMBED CHARTS: 6 PNG charts are available in {DATA_DIR}/viz/charts/:
  01_parker_cumulative_sales.png, 02_swoosh_13d_staleness.png,
  03_section16a_toggle.png, 04_buy_sell_divergence.png,
  05_anomaly_scatter.png, 06_litigation_timeline.png
  Use python-docx's add_picture() to embed relevant charts in the appropriate sections.
  Use Inches(6.0) width for landscape charts, Inches(5.5) for standard charts.
...
def run_phase_viz(targets=None, open_browser=False):
    """Phase 2.5: Generate all visualizations (charts, dashboard, deck)."""
    log("VIZ", "Starting visualization pipeline...", "PHASE")
    ...
    # Step 1: Static charts (always first — dashboard and deck embed these)
    log("VIZ", "[1/3] Generating 6 static charts (300 DPI PNG)...", "INFO")
    try:
        chart_paths = generate_all_charts()
        results["charts"] = len([v for v in chart_paths.values() if "ERROR" not in str(v)])
        log("VIZ", f"  {results['charts']}/6 charts generated", "OK")
    except Exception as e:
        log("VIZ", f"  Chart generation failed: {e}", "ERROR")
        results["charts"] = 0

    # Step 2: Interactive dashboard
    if targets is None or "dashboard" in targets:
        log("VIZ", "[2/3] Generating interactive HTML dashboard...", "INFO")
        try:
            dash_path = generate_dashboard(open_browser=open_browser)
            results["dashboard"] = str(dash_path)
            log("VIZ", f"  Dashboard: {dash_path}", "OK")
        except Exception as e:
            log("VIZ", f"  Dashboard failed: {e}", "ERROR")

    # Step 3: PPTX briefing deck
    if targets is None or "deck" in targets:
        log("VIZ", "[3/3] Generating 12-slide PPTX briefing deck...", "INFO")
        try:
            deck_path = generate_deck()
            if deck_path:
                results["deck"] = str(deck_path)
                log("VIZ", f"  Deck: {deck_path}", "OK")
            else:
                log("VIZ", "  Deck skipped (Node.js/pptxgenjs not available)", "WARN")
        except Exception as e:
            log("VIZ", f"  Deck failed: {e}", "ERROR")

    log("VIZ", "Phase 2.5 finished.", "OK")
    return results


async def run_full_pipeline():
    """Execute all phases in sequence with human gates."""
    log("PIPELINE", "Starting full pipeline run...", "PHASE")
    start_time = time.time()

    # Phase 1: Ingest
    log("PIPELINE", "═══ PHASE 1/5: INGEST ═══", "PHASE")
    await run_phase_ingest()

    # Phase 2: Cross-reference
    log("PIPELINE", "═══ PHASE 2/5: CROSS-REFERENCE ═══", "PHASE")
    await run_phase_crossref()

    # Phase 2.5: Visualization
    log("PIPELINE", "═══ PHASE 2.5/5: VISUALIZATION ═══", "PHASE")
    run_phase_viz(open_browser=False)

    # Phase 3: Reports (all four types — charts now available for embedding)
    log("PIPELINE", "═══ PHASE 3/5: REPORT GENERATION ═══", "PHASE")
    await run_phase_report("all")

    # Human checkpoint before submission
    print("\n" + "="*60)
    print("  PIPELINE CHECKPOINT: Phases 1-3 complete.")
    print(f"  Reports:   {DATA_DIR}/reports/")
    print(f"  Charts:    {DATA_DIR}/viz/charts/    (6 PNG files)")
    print(f"  Dashboard: {DATA_DIR}/viz/dashboard/  (open in browser)")
    print(f"  Deck:      {DATA_DIR}/viz/deck/       (12-slide PPTX)")
    print("  Review all outputs before proceeding to submission.")
    print("="*60)

    # Offer to open dashboard
    view_dash = input("\n  Open dashboard in browser? [yes/no]: ").strip().lower()
    if view_dash == "yes":
        import webbrowser
        dash_path = DATA_DIR / "viz" / "dashboard" / "jlaw_dashboard.html"
        if dash_path.exists():
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Commands:
  pipeline     Run all 5 phases (INGEST → CROSSREF → VIZ → REPORT → SUBMIT)
  ...
  viz          Phase 2.5: Generate charts, dashboard, and briefing deck
  ...
  status       Check data directory status and prerequisites
    ...
    parser.add_argument("command", nargs="?", default="status",
                       choices=["pipeline", "ingest", "crossref", "viz", "report", "submit",
                               "interactive", "status"])
    ...
    parser.add_argument("--viz-target", default=None, nargs="*",
                       choices=["charts", "dashboard", "deck"],
                       help="Viz targets for 'viz' command (default: all)")
    ...
    if args.command == "pipeline":
        ...
        run_phase_viz(targets=args.viz_target, open_browser=True)
